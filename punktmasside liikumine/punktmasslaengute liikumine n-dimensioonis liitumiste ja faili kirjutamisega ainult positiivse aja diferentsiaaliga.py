...
#m_prooton=;q_prooton=;m_elektron=;q_elektron=−1.602176565(35)×10−19 C
def küsi_punkt():
...
while len(l)>samm:
    ...
    while i<korduste_limiit:
        ...
        for p1 in range(0,len(l),samm):#võtab järjest punktmassi alguse indekseid
            for p2 in range(p1+samm,len(l),samm):#p2 on alati suurem kui p1
                ...
                for mõõde in range(a,samm,2):#kehade vahelise kauguse ruut = r**2
                    I_toorik+=(l[p1+mõõde]-l[p2+mõõde])**2#esimese punkti selle dimensiooni kordinaadi ja teise punkti selle dimensiooni kordinaadi vahe ruut
        ...
        if c:
            txt+="\n"
            if len(txt)>6000:#võib asendada i*dim*kehadearv#arvu võib muuta
                fail.write(txt.replace(";\n","\n"))
                txt=""
        if b:
            print("")
