...
class HidingScrollBar(ttk.Scrollbar):#see klass poleminu kirjutatud, vaid vist Andres Mihkelsoni kirjutatud.
    """
...
class Table(ttk.Frame):#see klass poleminu kirjutatud, vaid vist Andres Mihkelsoni kirjutatud.
    ...
    def clearTable(self):
        ...
        for row in self.labels:
            for label in row:
                label.destroy()
        self.labels = []##See klass pole minu tehtud.
...
def uuenda_algmoistete_loend():
    algmoistete_loend.delete(0,END)
    algmoistete_loend.insert(END, "proov")
    algmoistete_loend.insert(END, "proov2")
    algmoistete_loend.insert(END, "punktmasslaeng")
    algmoistete_loend.itemconfig(0, {'bg': 'royal blue'})
...
jou_mootmete_sisend=ttk.Entry(mudeli_frame)
...
N_sisend=ttk.Entry(vormi_frame1, width=3)
N_sisend.insert(0,"m_1")
N_sisend.grid(row=0,column=4)
ttk.Label(vormi_frame1, text=".").grid(column=5, row=0)
Button(vormi_frame1, text="lisa").grid(row=0, column=6)
vormi_frame1.grid(row=5, column=0, columnspan=2,sticky=(N,W))

seoste_lahter=Text(olukorra_frame,height=5, width=56, wrap=CHAR)
seoste_lahter.grid(row=6,column=0,columnspan=2,sticky=(N,W))
...
ttk.Label(valjundi_frame, text="Mõisted (algmõisted siniselt):").grid(row=2, column=0)
algmoistete_frame=Frame(valjundi_frame)
algmoistete_loend=Listbox(algmoistete_frame, height=4, width=45)
uuenda_algmoistete_loend()
scrollbar2=Scrollbar(algmoistete_frame,command=algmoistete_loend.yview)
scrollbar2.pack(side=RIGHT, fill=Y)
algmoistete_loend.config(yscrollcommand=scrollbar2.set)
algmoistete_loend.pack(side=LEFT)
algmoistete_frame.grid(row=3,column=0,columnspan=2)
valjundi_frame.grid(row=0, column=3, sticky=(N, W))
...
mudeli_info_frame.grid(row=1,column=0)
...
visualiseerimise_frame.grid(row=1,column=3,sticky=(N,W))
